...
class SawitSaleIntegration(models.Model):
    ...
    def create_sale_from_summaries(self, summary_ids, partner_id=None):
        ...
        # create a sale per mill (group by mill)
        grouped = {}
        for s in summaries:
        ...
        if not product:
            product = Product.create({'name': 'FFB', 'type': 'service', 'default_code': 'FFB'})
        for mill_id, sums in grouped.items():
            ...
            so = Sale.create({'partner_id': partner.id, 'partner_invoice_id': partner.id, 'partner_shipping_id': partner.id, 'date_order': fields.Datetime.now()})
            for s in sums:
                # find price entry for mill and date
                price = PriceModel.search([('mill_id','=',mill_id),('date','=',s.date)], limit=1)
                unit_price = price.price_per_kg if price else 0.0
                so.order_line.create({'order_id': so.id, 'product_id': product.id, 'name': 'FFB %s' % (s.date,), 'product_uom_qty': s.total_net_kg, 'price_unit': unit_price, 'product_uom': product.uom_id.id})
            so.action_confirm()
